...
public partial class SystemExplorerPlugin
{
    ...
    private void OnRefactorNamespaceConfirmed()
    {
        ...
        if (
            !NamespaceTextRewriter.IsValidNamespaceName(oldNamespace)
            || !NamespaceTextRewriter.IsValidNamespaceName(newNamespace)
        )
        {
            GD.PushWarning(
    ...
    private bool RefactorNamespace(string metadata, string oldNamespace, string newNamespace)
    {
        ...
        foreach (KeyValuePair<string, string> pendingWrite in pendingWrites)
        {
            if (!WriteTextFile(pendingWrite.Key, pendingWrite.Value))
            {
                ...
                ScriptResourceRefreshService.RefreshChangedScripts(pendingWrites.Keys);
                return false;
            }
        }
    ...
    )
    {
        ...
        string targetScriptPath = ScriptPathUtility.Normalize(
    ...
    )
    {
        ...
        {
            string selectedEntry = GetEntryFromMetadata(metadata);
            selectedScriptPath = ScriptPathUtility.Normalize(GetScriptPathFromEntry(selectedEntry));
        }
    ...
    private HashSet<string> GetRefactorNamespaceCandidateScriptPaths(string metadata)
    {
        ...
        return RefactorNamespaceScopeResolver
            .CombineScriptPaths(targetScriptPaths, GetRefactorNamespaceProjectCSharpFilePaths())
    ...
    private void DebugDumpRefactorNamespaceEditorState(string metadata, string label)
    {
        ...
        {
            string selectedEntry = GetEntryFromMetadata(metadata);
            targetPath = ScriptPathUtility.Normalize(GetScriptPathFromEntry(selectedEntry));
        }
        ...
        DebugLog(
            $"Refactor Namespace editor diagnostics ({label}): target='{targetPath}', openScripts={openScripts.Count}, openScriptEditors={openScriptEditors.Count}, currentScript='{ScriptPathUtility.Normalize(currentScript?.ResourcePath)}', currentEditorId={GetGodotInstanceId(currentEditor)}, currentBaseType='{currentBaseEditor?.GetType().Name ?? "<null>"}', currentBaseId={GetGodotInstanceId(currentBaseEditor)}"
        );
        ...
        for (int i = 0; i < openScripts.Count; i++)
        {
            ...
            DebugLog(
                $"Refactor Namespace open script [{i}]: path='{ScriptPathUtility.Normalize(openScript?.ResourcePath)}', id={GetGodotInstanceId(openScript)}"
            );
        }
