...
internal sealed class NamespaceRefactorDialogConfirmationCoordinator
{
	...
	)
	{
		...
		_isValidNamespaceName =
			...
			?? throw new ArgumentNullException(nameof(isValidNamespaceName));
	...
	private void ConfirmSingleReplacement(string newNamespace)
	{
		...
		_logOperation(
			"Refactor Namespace Confirmed",
			$"{oldNamespace} -> {newNamespace}"
		);

		if (
			!_isValidNamespaceName(oldNamespace)
			|| !_isValidNamespaceName(newNamespace)
		)
		{
			_showWarning(
	...
	private void ConfirmBatchAdd(string newNamespace)
	{
		...
		List<string> targetScriptPaths = _sessionState
			.ScriptPaths.Where(path =>
				string.IsNullOrWhiteSpace(_readNamespaceFromScript(path))
			)
	...
	private void ConfirmBatchReplacement(string newNamespace)
	{
		...
		{
			_debugLog(
				"Refactor Namespace batch cancelled: no valid old namespace was selected."
			);
			return;
		}
