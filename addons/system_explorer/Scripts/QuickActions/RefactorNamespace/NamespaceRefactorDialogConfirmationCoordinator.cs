...
internal sealed class NamespaceRefactorDialogConfirmationCoordinator
{
	...
	)
	{
		...
		_readNamespaceFromScript = readNamespaceFromScript ?? throw new ArgumentNullException(nameof(readNamespaceFromScript));
	...
	private void ConfirmBatchAdd(string newNamespace)
	{
		...
		diagnosticContext.Log("Request", "Confirmed operation started.");
		_addNamespace(diagnosticContext, targetScriptPaths, newNamespace, "Refactor Namespace Batch Add");
		_sessionState.Clear();
	}
