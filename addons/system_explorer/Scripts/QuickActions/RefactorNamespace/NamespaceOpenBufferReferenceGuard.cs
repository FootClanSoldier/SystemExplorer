...
internal sealed class NamespaceOpenBufferReferenceGuard
{
	internal NamespaceOpenBufferReferenceGuardResult FindUnsafeReference(
		...
		IReadOnlyDictionary<string, OpenScriptEditorBufferGroup> matchedOpenEditorGroupsByPath,
		string namespaceName
	)
	{
		if (
		...
		HashSet<TextEdit> matchedTextEditors =
			matchedOpenEditorGroupsByPath
				?.Values.Where(group => group != null)
				.SelectMany(group => group.Buffers)
				.Select(buffer => buffer.TextEditor)
