...
Den tidigare TextChanged-cancellation-A/B-flaggan är nu pensionerad i completion-ownership-foundationen. Managed session/state transitions avgör fortfarande om completion blir dormant/stale, men en deferred TextChanged får bara native-cancella när `AutocompleteCodeEditMutationCoordinator` kan bevisa en current owned publication lease för exakt current `EditorBindingLease`/CodeEdit/path. Ingen publication lease betyder noll native cancellation. Detta är en partial permanent foundation ovanpå den redan implementerade lifecycle-state-machinen.
...
Detta är **en bra permanent princip**, med en viktig begränsning: metadata är en ownership ledger, inte ett lås. `SetMeta`, prefixändringar och theme overrides är fortfarande native mutationer. De måste i slutarkitekturen gå genom samma binding/mutation coordinator som övriga CodeEdit-operationer. `BeginBulkThemeOverride/EndBulkThemeOverride` kan minska theme-notification-churn men skapar inte i sig någon managed-reload- eller ScriptEditor-lifecycle-barriär. I den nu implementerade completion-foundationen lämnas `AutocompleteCodeEditNativeOwnershipBridge` funktionellt separat: completion request/publication leases lagras inte i native metadata-ledgern, och bridge:n får inga PublicationId/BindingEpoch/session/request-transaction fields.
...
| Deferred TextChanged validation utan owned publication | **Permanent fail-closed baseline.** Ingen owned publication lease betyder noll native cancellation; owner-scoped cancellation är tillåten först när exact current binding/publication ownership bevisas. |
...
**Owner-scoped Cancel under deferred TextChanged validation — [Partial foundation implementerad].** Godots TextChanged är redan deferred, men cancellation kan fortfarande konkurrera med ny completion request, script transition eller deferred using. Därför är generell cancellation fortfarande förbjuden: den nya coordinatorn får endast cancel:a en exakt current owned publication, consume:ar ownership före native callen och gör ingenting när ownership inte kan bevisas.
...
> Ingen `ConfirmCodeCompletion`, `CancelCodeCompletion`, `RequestCodeCompletion`, `Add/UpdateCodeCompletionOptions`, `InsertText`, caretmutation, prefix/theme/meta-mutation eller signal-rebind får exekveras från en feature bara för att `TryGetActiveCodeEdit()` nyss returnerade true. Native mutation/dispatch ska vara coordinator-owned och verifierad mot current lifecycle authority. För Publish, OwnedCancel, Confirmation och deferred automatic-using `InsertText` betyder det nu exklusiv `StableBindingMutation`; ordinary `RequestCodeCompletion` är avsiktligt ett undantag från just den exklusiva slotten tills ett reentrancy-safe RequestDispatch parent/child-protokoll finns, eftersom native request-callen legitimt kan ge en synkron `CodeCompletionRequested` child callback som måste få publicera.

### Implementationsstatus efter post-reload stabilization foundation

**Nu permanent implementerat:** den explicita autocomplete-orderingen är `ManagedAssemblyGeneration → reload readiness / ReloadReadyEpoch → ScriptTransitionId → candidate stability proof när policyn kräver det → BindingEpoch`. `AutocompleteReloadStabilizationCoordinator` är pure-managed och separat från `ScriptEditorLifecycleCoordinator`: fresh startup börjar `Ready` med epoch 1, medan `ResetManagedAssemblyTransientStateAfterReload()` gör epoch unavailable och sätter `ReloadQuiescent`. Den första C#-activationen efter reload stabiliseras fortsatt enbart av reload-coordinatorn över två separata `_Process`-turns; dess exact candidate skickas direkt till den befintliga deferred resolvern och dubbelstabiliseras inte. Först efter lyckad BindingEpoch-commit publiceras reload state `Ready` med `ReloadReadyEpoch > 1`.

**Post-reload ordinary C# ScriptTransition stabilization — IMPLEMENTED FOUNDATION:** när generationen därefter är `Ready` med `ReloadReadyEpoch > 1` får varje senare known-C# `BindingPending` transition ett separat pure-managed candidate-stability proof via `AutocompleteScriptTransitionStabilizationCoordinator`. Första deferred rebind-försöket arm:ar endast `{HostInstanceToken, ScriptTransitionId}` och returnerar utan BindingActivation. Central `_Process` observerar sedan exakt current candidate högst en gång per invocation; samma ManagedAssemblyGeneration, HostInstanceToken, ScriptTransitionId, ScriptEditor/ScriptEditorBase/CodeEdit IDs och normalized path måste observeras på två separata process-turns. Candidate change startar om beviset, och en ny ScriptTransitionId gör gammal authority omedelbart oanvändbar. Navigationen fördröjs inte: `EditScript()` och Navigation Stress fortsätter i samma takt, medan autocomplete-binding coalescas latest-wins så transient intermediate candidates kan hoppas över helt. Authorized ordinary candidate återanvänds som `requiredActivationCandidate`, får samma final exact revalidation i `ResolveCodeEditBinding(...)`, och den committed `EditorBindingLease` måste därefter bevisa samma candidate och current ReloadReadyEpoch. Failure/mismatch kräver två nya observationer; ingen timer, delay, timeout eller native cancellation ingår.

**Same-script/no-signal System Explorer transition reconciliation — IMPLEMENTED:** en konkret lifecycle-lucka är permanent stängd. System Explorer invalidaterar fortfarande outgoing BindingEpoch genom att börja en riktig `SystemExplorerNavigation` transition före exakt ett `EditScript(...)`. Om target redan var Godots current script före anropet kan Godot däremot returnera från same-script `EditScript` utan `editor_script_changed`; tidigare kunde lifecycle då ligga kvar i `ScriptTransitionPending` på obestämd tid eftersom generic rebind recovery medvetet vägrar att forcera en pågående System Explorer-navigation till BindingPending. Script-open-vägen gör därför nu en read-only pre-call path comparison endast för att identifiera detta fall och queue:ar efter returned `EditScript` en one-shot deferred exact observation som bär endast `{ManagedAssemblyGeneration, ScriptTransitionId, ExpectedScriptPath}`. Callbacken avvisar stale/superseded work före current-script-read, kräver därefter att actual active path exakt matchar expected target, revaliderar samma transition efter native read och använder coordinatorns exact expected-target operation för att göra endast `ScriptTransitionPending → BindingPending`; om den riktiga signalen redan hann göra samma transition BindingPending säkerställs bara den befintliga deferred rebind-intenten. Ingen polling, timeout, retry, second EditScript, direct BindingActivation eller CodeEdit-mutation införs. Post-reload ordinary two-process-turn stabilization gäller därför fortfarande efter reconciliation när `ReloadReadyEpoch > 1`; fresh startup epoch 1 behåller ordinary admission.

> **Invariant:** `editor_script_changed` är en observation, inte ett garanterat completion-event. För en System Explorer-owned transition där target redan var current före `EditScript`, får en generation-/transition-bound post-return exact active-path observation slutföra samma transition till BindingPending när signalen uteblir.

`ScriptEditorLifecycleCoordinator` fortsätter samtidigt äga exakt `Detached / ScriptTransitionPending / BindingPending / Stable`, current ScriptTransitionId och BindingEpoch. `AutocompleteReloadStabilizationCoordinator` äger endast första post-reload readiness och utfärdar ReloadReadyEpoch; `AutocompleteScriptTransitionStabilizationCoordinator` äger endast senare ordinary candidate proof; `AutocompleteCodeEditMutationCoordinator` äger fortsatt concrete native mutation execution. ReloadReadyEpoch, ScriptTransitionId och BindingEpoch flyttas inte in i mutation coordinatorn. Fresh startup `ReloadReadyEpoch=1`, non-C# targets och pathless recovery behåller tidigare ordinary binding semantics.

**PARTIAL FOUNDATION IMPLEMENTED:** en enda växande `AutocompleteCodeEditMutationCoordinator` äger nu completion request/publication protocol, normal binding activation/presentation och stable-binding native execution för **publication, owner-scoped cancellation, confirmation och deferred automatic-using InsertText**. Completion request admission/collection förblir pure/managed preparation. Efter ordinary pre-publish anchor check öppnar Publish den generella immutable `AutocompleteStableBindingMutationLease`, gör final request/binding/anchor validation, håller slotten genom presenter Add*/Update, revaliderar efter returned Update och registrerar PublicationLease innan slotten släpps. OwnedCancel öppnar samma lease-typ, revaliderar samma publication efter admission, consume:ar ownership, gör final exact binding validation och kör därefter native Cancel. Confirmation behåller sin consume-before-confirm + final binding revalidation. DeferredUsingInsertText behåller CallDeferred-stackgränsen och alla outer generation/host/barrier-guards; efter hostens exact current-binding precheck öppnar coordinatorn samma `AutocompleteStableBindingMutationLease`, revaliderar exact originating EditorBindingLease/current authority, bygger execution context från transaction-leasen och håller slotten genom service-owned InsertText. Contention/rejection failar terminalt utan retry/requeue. Binding activation använder fortsatt separat immutable `AutocompleteBindingActivationTransactionLease`; inom den transactionen reserveras nu BindingEpoch före första reversible presentation-write och samma reservation används för schema-v2 marker + final lease commit. BindingActivation och StableBindingMutation är ömsesidigt exklusiva och Publish/OwnedCancel/Confirmation/DeferredUsingInsertText delar samma monotona MutationTransactionId-sekvens. ReloadReadyEpoch, ScriptTransitionId och BindingEpoch förblir lifecycle-inputs/bevis och flyttas inte in i mutation coordinatorn. Ordinary `RequestCodeCompletion` är fortfarande coordinator-owned method/authority men **inte** exclusive StableBindingMutation execution. Completion-source isolation, callback fault containment, host circuit breaker och reversible native presentation ownership är nu implementerade foundations. **Nästa övergripande hardening är publication anchor/liveness ownership; därefter `RequestCodeCompletion` parent/child `RequestDispatch`, sedan `ExternalMutationLease`, shutdown special path och övriga CodeEdit/TextEdit mutationer.**

**Current permanent order after BindingEpoch-owned presentation ownership:**

```text
IMPLEMENTED:
- reload stabilization
- post-reload ordinary transition stabilization
- same-script/no-signal transition reconciliation
- BindingActivation mutation authority
- BindingEpoch-owned reversible native presentation
- Publish / OwnedCancel / Confirmation / DeferredUsingInsertText
- completion-source isolation + callback fault containment + host circuit breaker

NEXT:
- publication anchor/liveness ownership

THEN:
- RequestCodeCompletion parent/child RequestDispatch authority

THEN:
- ExternalMutationLease

THEN:
- shutdown/remaining CodeEdit/TextEdit mutation paths
```
...
| **Post-reload stabilization / hård cold-generation readiness — IMPLEMENTERAD FOUNDATION** | **Permanent implementerat:** en managed recovery returnerar inte längre implicit autocomplete-native readiness. `ResetManagedAssemblyTransientStateAfterReload()` sätter separat `ReloadQuiescent` authority och gör ReloadReadyEpoch unavailable; startup utan reload börjar Ready. | `AutocompleteReloadStabilizationCoordinator` äger endast pure-managed generation/state/token/epoch/candidate IDs/path. Befintlig `_Process` observerar current C# candidate read-only över två separata turns. Första post-reload deferred activation kräver samma generation + host + ScriptTransitionId + ScriptEditor/ScriptEditorBase/CodeEdit IDs + normalized path, current ReloadReadyEpoch och en BindingEpoch lease som bär samma epoch. Candidate churn/failure restartar fail-closed. Managed recovery/full rebuild fallback får slutföras medan barriären fortfarande är stängd. | **Verifieringsgate kvar i Godot:** rebuild + churn + Navigation Stress samt ungefär 100 sequential rebuilds i långlivade processer. Ingen BindingEpoch eller feature-native CodeEdit mutation får etableras före ActivationPending/current epoch. Efter verifiering är nästa större roadmap-steg CodeEdit mutation/transaction coordinatorn, inte fler lokala reload-bools. |
| **Post-reload ordinary C# ScriptTransition stabilization — IMPLEMENTED FOUNDATION** | **Evidens/riktning:** första reload-bindingen var redan two-turn-stabilized, men senare `BindingPending` transitions i samma post-reload generation gick direkt från deferred callback till BindingActivation. Post-reload Navigation Stress var den tydligaste kvarvarande reproduktionsförstärkaren. **Mål:** transient intermediate CodeEdit ska inte få signal/prefix/theme/meta/binding ownership bara för att `EditScript()` redan returnerat. | `AutocompleteScriptTransitionStabilizationCoordinator` är pure-managed och separat från både reload readiness och mutation authority. Endast när reload är `Ready`, epoch > 1, lifecycle är current BindingPending och authoritative ObservedScriptPath annars ExpectedScriptPath är known `.cs`, arm:as ordinary proof. `_Process` gör högst en befintlig read-only candidate query per invocation; två authority-equal observations krävs. Authorization queue:ar endast den befintliga deferred resolvern, som skickar candidate till `ResolveCodeEditBinding(requiredActivationCandidate)` och därefter verifierar committed lease. Fresh startup epoch 1, första reload activation, non-C# och pathless recovery bypassar denna barrier. | **Verifieringsgate:** rebuild → Ready → 250+/500+/1000+ Navigation Stress med oförändrad 75 ms cadence; intermediate transitions får legitimt sakna BindingEpoch. Stress → stop → autocomplete → Enter och autocomplete-only över flera rebuilds ska behålla Publish/Confirmation/DeferredUsing StableBindingMutation behavior. Nästa crash-tail ska kunna skilja `EditScript Returned` utan BindingActivation från crash inne i eller efter stabilized BindingActivation. |
| **Explicit ScriptEditor/CodeEdit binding state machine — IMPLEMENTERAD FOUNDATION** | **Permanent implementerat:** implicit callback-depth/bool-logik är ersatt som ScriptEditor correctness authority av `ManagedAssemblyGeneration + ScriptTransitionId + BindingEpoch`, och varje BindingEpoch bär nu den externa ReloadReadyEpoch som var current vid commit. System Explorer-navigation börjar transition före exakt ett `EditScript`; `editor_script_changed` är primary expected/external-path observation men inte ett garanterat completion-event. För ett target som redan var current före EditScript finns nu generation-/ScriptTransitionId-bound post-return exact reconciliation: om samma transition fortfarande är current och actual active path exakt matchar expected target får den endast gå `ScriptTransitionPending → BindingPending`. Repeated same-target notifications i `BindingPending` behåller samma transition medan en verklig target-förändring supersedar den; varje faktiskt ny transition gör outgoing lease stale direkt. | `EditorIntegration/ScriptEditing/ScriptEditorLifecycleCoordinator`, `SystemExplorerPlugin.ScriptEditorLifecycle`, `ScriptOpening`, `ScriptEditorSync`, `SystemExplorerPlugin.Autocomplete`, `AutocompletePluginHost`, `AutocompleteEditorBinding`. Coordinatorns normal signalväg och exact deferred same-script-väg delar samma expected-target state-transition core; exact operationen kan aldrig BeginTransition/superseda/acceptera annan target. Latest-wins deferred resolver verifierar generation + host + operation token + current transition före editor access och är fortfarande enda CodeEdit binding-establishment path. Immutable lease innehåller ReloadReadyEpoch + native IDs/path; stale candidate rollback publiceras aldrig. Normal rebind **cancel:ar inte native completion**. | **Verifieringsgate kvar i Godot:** same-script click utan virtual move, same-script efter virtual move, restart med target redan current, post-reload same-script, delayed real-signal coexistence samt långsam/snabb/filtered/held-arrow/Create Script/external ScriptEditor navigation och rebuild-stress. Outgoing BindingEpoch får aldrig bli current igen, different-script navigation ska inte få reconciliation-callbacken, och post-reload same-script måste fortfarande passera ordinary two-turn candidate proof. Den statiska arkitekturprincipen är permanent; BindingEpoch-allokeringen är nu dessutom tvåfasig via en pending EditorBindingReservation som reserveras före reversible presentation och committas exakt först efter successful activation. |
| **En enda CodeEdit mutation/transaction coordinator — PARTIAL FOUNDATION IMPLEMENTED** | **Implementerat i den utökade partiala foundationen:** `AutocompleteCodeEditMutationCoordinator` äger completion request/publication protocol, exact binding activation/presentation transaction och exact stable-binding execution för **Publish, OwnedCancel, Confirmation och DeferredUsingInsertText**. Publish går ordinary pre-validation → StableBindingMutation admission → final anchor/authority validation → presenter Add*/Update → post-publish authority/anchor validation → PublicationLease acquisition → release. OwnedCancel går publication verification → StableBindingMutation admission → same-publication revalidation → consume → final exact binding validation → native Cancel → release. DeferredUsingInsertText går outer deferred guards → exact current binding precheck → StableBindingMutation admission → final exact binding/originating-lease validation → service InsertText → release, utan retry/requeue. | Ordinary feature-owned `RequestCodeCompletion` går fortsatt genom coordinator-metoden men håller avsiktligt inte StableBindingMutation över native dispatch, eftersom request-callen kan ge en synkron legitim `CodeCompletionRequested` child callback. **Reversible native presentation ownership är nu implementerad foundation utan ny mutation-lease:** BindingActivation reserverar BindingEpoch före presentation, schema-v2 markern bär exakt owner identity + Previous/Applied state och commit återanvänder samma reservation. **Återstår i prioriterad ordning:** publication anchor/liveness policy; reentrancy-safe RequestDispatch parent/child protocol; `ExternalMutationLease`; shutdown special path; remaining CodeEdit/TextEdit mutations. | **Verifieringsgate:** publish/cancel/deferred-InsertText diagnostics ska bära nonzero MutationTransactionId; ingen BindingActivation/competing stable mutation får starta medan Publish/OwnedCancel/Confirmation/DeferredUsingInsertText äger slotten. Request-owned dormant/forced follow-up måste fortfarande kunna dispatch → synchronous callback → publish utan StableBindingMutationActive self-rejection. |
| **Completion transaction protocol — FOUNDATION IMPLEMENTED** | **Implementerat:** exact request lease skapas efter prefix/line-text capture; source collection är pure/managed; ordinary exact binding/caret/line revalidation sker före native transaction admission. Publish öppnar därefter StableBindingMutation och gör en andra final request/binding/anchor check innan första Add; presenter kör Add* + exakt en Update under samma slot; post-Update revalidation sker innan PublicationLease skapas och innan slot release. | TextChanged kan endast cancel:a current owned publication. OwnedCancel öppnar StableBindingMutation, revaliderar samma publication efter admission, consume:ar före native Cancel och gör final exact binding/CodeEdit/path validation efter consume; caret/line anchor krävs inte för cancellation. Keyboard confirmation behåller metadata ownership + current publication ownership + StableBindingMutation + consume-before-confirm. Deferred using bär originating PublicationId + exact EditorBindingLease; hosten gör read-only exact current-binding precheck och coordinatorn håller StableBindingMutation(Operation='DeferredUsingInsertText') genom service-owned InsertText. Ordinary RequestCodeCompletion är coordinator-owned dispatch men ännu inte exclusive stable execution. | **Verifiering:** popup open/typing/backspace/accept, repeated requests, dormant recovery/forced follow-up, scriptbyte under collection/publish/cancel/confirmation/deferred insertion, rebuild och stale deferred using. Publish/cancel/deferred InsertText Begin/Returned ska bära samma nonzero MutationTransactionId inom respektive operation. |
| **Completion source isolation + callback fault containment — IMPLEMENTED** | **Permanent implementerat:** completion source identity är immutable registration metadata (`ProjectType`, dormant `ProjectMember`) och coordinatorn kopierar/validerar registryt vid construction. Hela source-pathen — registration/source validation, GetCompletions, returned collection, enumeration/filter/copy/AddRange — är source-isolerad. Unexpected host-level completion-pipeline exceptions circuit-breakar exakt current AutocompletePluginHost/HostInstanceToken/ManagedAssemblyGeneration och får inte lämna hostens callback. | Fault path gör endast managed fail-closed cleanup: pending member follow-up clear, completion validation/session invalidation och logical PublicationLease retirement. Ingen Cancel/Request/Confirm/Add/Update/InsertText, binding resolution, Shutdown, theme/prefix/meta restore eller annan native cleanup körs från fault-pathen. Faulted host förblir lifecycle/binding owner men all System Explorer completion processing — collection/publication/owned cancel/confirmation/deferred using/dormant+forced request work/TextChanged validation — är inert tills cold host recreation. `OnAutocompleteCodeCompletionRequested()` är dessutom sista plugin-owned `catch (Exception)` boundary mot Godots signal bridge och cancel:ar pending deferred using pure-managed när current host faultar. | **Verifieringsgate:** återproducera log_78-sekvensen. Vid unexpected pipeline fault ska exakt en fault-rad visas, callback returnera normalt och senare requests ge zero collection/request transaction/Publish/Add/Update/Confirm/InsertText samt högst en suppression-rad per host. Nytt script får inte re-enable samma host; rebuild/new generation/new HostInstanceToken får skapa en healthy host. Source-specific throw ska däremot endast ge `completion source failed` och fortsatt collection returned utan host fault. |
...
| **Reversible native presentation ownership — IMPLEMENTED FOUNDATION** | **Permanent implementerat:** reversible CodeEdit presentation ägs nu av exakt den BindingEpoch som skapade den. BindingEpoch reserveras i `BindingPending` före första prefix/theme-write och reservationen är inte Stable authority. | `AutocompleteCodeEditNativeOwnershipBridge` behåller historical metadata key men läser schema v1 + v2 och skriver endast v2. V2 owner identity är ManagedAssemblyGeneration + HostInstanceToken + ScriptTransitionId + ReloadReadyEpoch + BindingEpoch + CodeEdit ID + normalized ScriptPath och marker-state bär Previous + Applied presentation. Outgoing/stale/reload cleanup gör compare-before-restore; current != Applied lämnas orört och räknas som safe relinquish, medan native failure failar closed och bevarar markern. Legacy v1 migreras genom härledd Applied-state under BindingActivation. | **Verifieringsgate i Godot:** v1→v2 hot reload, v2→v2 reload, external prefix/theme changes, ordinary + post-reload Navigation Stress och stress→autocomplete. Varje committad presentation-owning BindingEpoch ska ha exakt matching v2 marker under aktiv ownership. Hard-crash-frånvaro är sekundär evidens. |
...
Den permanenta authority-kedjan är nu `ManagedAssemblyGeneration → ReloadReadyEpoch → ScriptTransitionId → optional post-reload ordinary candidate proof → BindingActivation → reserved BindingEpoch → reversible presentation marker v2 → exact BindingEpoch commit → StableBindingMutation`, med same-script/no-signal exact transition reconciliation som en smal observation fallback inom ScriptTransition authority och ett separat completion-pipeline health/circuit-breaker-lager på hostnivå ovanför feature mutation admission. Första reload-activation och senare ordinary post-reload C# transitions har separata state machines men återanvänder samma immutable candidate identity och samma final exact binding revalidation. Den gemensamma mutation authorityn omfattar normal BindingActivation samt stable execution för **Publish, OwnedCancel, Confirmation och DeferredUsingInsertText**; completion-source isolation + callback containment + host circuit breaker är implementerat. Reversible native presentation ownership är nu **IMPLEMENTED FOUNDATION** genom BindingEpoch reservation, schema-v2 exact owner identity, compare-before-restore, safe relinquish när current native state ändrats och legacy schema-v1 migration. **NEXT är publication anchor/liveness ownership. THEN ett reentrancy-safe `RequestCodeCompletion` RequestDispatch parent/child-protokoll. THEN `ExternalMutationLease`. THEN shutdown-special-path och remaining CodeEdit/TextEdit mutation paths.**
...
**Request/cancel behavior** är nu delvis permanentad. Owner-scoped native cancellation är StableBindingMutation-owned i `Stable` state och consume-before-cancel. Ordinary `RequestCodeCompletion` är däremot endast coordinator-owned dispatch authority, inte exclusive StableBindingMutation execution. För att serialisera request korrekt krävs senare ett parent `RequestDispatch`-protokoll som uttryckligen auktoriserar den synkrona `CodeCompletionRequested` child callbacken och dess child publication; en naiv stable slot över hela native request-callen skulle annars blockera operationens egen förväntade callback. Publication anchor/liveness ownership prioriteras nu före detta RequestDispatch-steg.

**Automatic using** är nu permanent integrerad i transaction coordinator för den aktiva deferred modellen. Behåll:
...
Managed rebuild
    ↓
new ManagedAssemblyGeneration
    ↓
ReloadQuiescent / ReloadReadyEpoch unavailable
    ↓
managed integration recovery may complete
    ↓
two separate read-only process observations of one current candidate
    ↓
ActivationPending / new ReloadReadyEpoch
    ↓
existing deferred ScriptEditor resolver revalidates exact candidate
    ↓
BindingActivationTransaction admitted for exact generation + host + ScriptTransitionId + ReloadReadyEpoch
    ↓
outgoing cleanup → candidate recovery → signal/prefix/theme/meta activation
    ↓
new ScriptTransition-bound BindingEpoch carrying ReloadReadyEpoch
    ↓
BindingActivationTransaction released
    ↓
Reload stabilization Ready
    ↓
completion request/publication transaction protocol
    ↓
implemented: deferred InsertText stable execution + completion-source/callback containment + same-script/no-signal transition reconciliation + BindingEpoch-owned reversible native presentation → next: publication anchor/liveness → then: RequestCodeCompletion parent/child RequestDispatch authority → ExternalMutationLease → shutdown/remaining CodeEdit/TextEdit mutation

editor_script_changed (primary observation)
    ↓
...
System Explorer EditScript(target already current)
    ↓
exact same transition begins; outgoing BindingEpoch stale
    ↓
EditScript returns without guaranteed editor_script_changed
    ↓
one generation-/ScriptTransitionId-bound deferred active-path observation
    ↓ exact current transition + exact target only
BindingPending
    ↓
existing deferred resolver / post-reload stabilization policy
    ↓
new BindingEpoch
