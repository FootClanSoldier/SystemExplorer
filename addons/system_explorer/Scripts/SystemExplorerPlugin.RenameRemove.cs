...
public partial class SystemExplorerPlugin
{
    ...
    private void OnRemoveConfirmed()
    {
        ...
        if (removeFromFilesystem)
        {
            ...
            if (!deleteResult.Success)
            {
                ...
                DebugLogOperation("Physical remove failed", deleteResult.FailureMessage);
                return;
            }
        }
            ...
            foreach ((string scriptPath, List<Script> matches) in matchingScriptsByPath)
            {
                ...
                if (
                    !TryAutosaveOpenScriptEditorBufferIfNeeded(
                        ...
                        didAutosaveOpenEditor: out _,
                        failureMessage: out string autosaveFailureMessage,
                        operationName: "Remove Script"
                    )
                )
                {
    ...
    private PhysicalDeleteResult DeleteFile(string resourcePath, List<string> deletedPaths)
    {
        ...
        if (FileAccess.FileExists(uidPath))
        {
            ...
            DebugLogOperation("Delete File: removed uid sidecar", uidPath);
        }
    ...
    private bool RenameScript(string metadata, string newName)
    {
        if (_pendingScriptRenameEditorRestore != null)
        {
            ...
            DebugLogOperation("Rename Script blocked: previous editor restore pending");
        ...
        if (treeState == null || !treeState.IsValid)
        {
            ...
            DebugLogOperation("Rename Script failed: selected tree identity unavailable", entry);
            return false;
        }
        ...
        if (matchingOpenScripts.Count != 1)
        {
            ...
            DebugLogOperation("Rename Script failed: target script tab not open", oldScriptPath);
            return false;
        }
        ...
        try
        {
            ...
            if (
                !TryAutosaveOpenScriptEditorBufferIfNeeded(
                    ...
                    didAutosaveOpenEditor: out _,
                    failureMessage: out string autosaveFailureMessage,
                    operationName: "Rename Script"
                )
            )
            {
                GD.PushWarning(autosaveFailureMessage);
                DebugLogOperation("Rename Script failed: autosave", autosaveFailureMessage);
                TryRestorePreviouslyActiveScript(
                    editorInterface,
                    scriptEditor,
                    activeScriptBeforeOperation,
            ...
            if (!closeResult.Success)
            {
                ...
                DebugLogOperation("Rename Script failed: close tab", closeFailureMessage);
    ...
    private void CompletePendingScriptRenameEditorRestore(bool succeeded, string failureMessage)
    {
        ...
        try
        {
            if (!succeeded)
            {
                ...
                DebugLogOperation("Rename Script deferred editor restore failed", warningMessage);
            }
            else
            {
                DebugLogOperation(
        ...
        {
            DebugLogOperation("Rename Script terminal restore warning", exception.Message);
        }
    ...
    )
    {
        ...
        if (editorInterface == null || scriptEditor == null)
        {
            ...
            DebugLogOperation("Rename Script failed: Script Editor unavailable", scriptPath);
            return false;
        }
    ...
    )
    {
        ...
        DebugLogOperation("Rename Script failed: destination UID sidecar exists", newUidPath);
        return false;
    }
    ...
    )
    {
        ...
        {
            DebugLogOperation("Rename Script: moved uid sidecar", $"{oldUidPath} -> {newUidPath}");
            return true;
        }
