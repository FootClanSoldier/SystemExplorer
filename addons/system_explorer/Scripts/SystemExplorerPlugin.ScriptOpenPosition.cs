...
public partial class SystemExplorerPlugin
{
	...
	)
	{
		...
		string normalizedScriptPath = NormalizeScriptOpenPositionPath(scriptPath);

		if (string.IsNullOrWhiteSpace(normalizedScriptPath))
			normalizedScriptPath = NormalizeScriptOpenPositionPath(script.ResourcePath);
	...
	{
		string normalizedExpectedPath = NormalizeScriptOpenPositionPath(expectedScriptPath);
		...
		string currentScriptPath = NormalizeScriptOpenPositionPath(currentScript?.ResourcePath);
	...
	private void MoveTextEditorToTop(TextEdit textEditor, string scriptPath)
	{
		...
		try
		{
			...
			DebugLog($"Script open top reset applied to '{scriptPath}'.");
		}
	...
	private static string NormalizeScriptOpenPositionPath(string path)
	{
