    ...
    )
    {
        ...
        foreach (string part in parts)
        {
            ...
            if (!folders.ContainsKey(currentPath))
            {
                ...
                if (TryGetFolderBinding(systemName, currentPath, out string boundFolderPath))
                {
                    folderItem.SetIconModulate(0, _boundFolderColor);
                    folderItem.SetTooltipText(0, boundFolderPath);
                }
                else
                {
                    folderItem.SetIconModulate(0, _folderColor);
                    folderItem.SetTooltipText(0, "");
                }
