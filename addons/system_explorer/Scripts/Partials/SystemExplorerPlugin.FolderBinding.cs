...
public partial class SystemExplorerPlugin
{
		...
		try
		{
			...
			foreach (JsonProperty systemProperty in document.RootElement.EnumerateObject())
			{
				...
				if (systemBindings.Count > 0)
					loadedFolderBindings[systemName] = systemBindings;
			}
		}
		catch (Exception exception)
		{
	...
	private bool SaveFolderBindings()
	{
		...
		try
		{
			SortedDictionary<string, SortedDictionary<string, string>> serializedBindings =
				new(StringComparer.Ordinal);

			foreach (KeyValuePair<string, Dictionary<string, string>> systemBinding in _folderBindings)
			{
	...
	)
	{
		...
		string physicalParentFolderPath = NormalizeBoundFolderPath(
			physicalFilePath.GetBaseDir()
		);
	...
	private bool TryGetFolderBindingFromMetadata(
		string metadata,
		out string physicalFolderPath
	)
	{
	...
	private void OnFolderBindingDirectorySelected(string selectedDirectory)
	{
		...
		if (
			!TryPersistReversibleSystemsAndFolderBindingsMutation(
				snapshot,
				systemsChanged: addedEntries > 0,
				folderBindingsChanged: true,
				operationName: "Bind Folder"
			)
		)
		{
			if (!HasActiveTreeOperationFailure)
	...
	private int SynchronizeAllBoundFolders()
	{
		...
		)
		{
			...
			foreach (
				KeyValuePair<string, string> folderBinding in systemBinding.Value
					.OrderBy(binding => binding.Key, StringComparer.OrdinalIgnoreCase)
					.ThenBy(binding => binding.Key, StringComparer.Ordinal)
					.ToList()
			)
	...
	private void ReportUnavailableBoundFolder(
		string physicalFolderPath,
		bool showUserWarnings
	)
	{
	...
	private bool MigrateFolderBindingsForSystemRename(
	...
	private bool MigrateFolderBindingsForFolderRename(
		string systemName,
	...
	)
	{
		...
		foreach (KeyValuePair<string, string> binding in affectedBindings)
		{
			string migratedFolderPath = binding.Key == oldFolderPath
				? newFolderPath
	...
	private bool RemoveFolderBindingsForSystem(string systemName)
	{
	...
	private bool RemoveFolderBindingsForFolderAndDescendants(string metadata)
	{
