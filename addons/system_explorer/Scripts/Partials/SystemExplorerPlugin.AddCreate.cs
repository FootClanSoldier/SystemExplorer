...
public partial class SystemExplorerPlugin
{
	...
	private void OnScriptFilesSelected(string[] paths)
	{
		...
		{
			PushSystemExplorerWarning("Select a system or folder before adding a script.", mirrorToDebugLog: false);
			DebugLogger.LogOperation(
				"Warning: Add Existing Scripts cancelled: no selected destination",
				string.Join(", ", paths)
			);
			return;
		}
	...
	private void OnSceneFilesSelected(string[] paths)
	{
		...
		if (string.IsNullOrWhiteSpace(GetSelectedSystemName()))
		{
			PushSystemExplorerWarning("Select a system or folder before adding a scene.", mirrorToDebugLog: false);
			DebugLogger.LogOperation(
				"Warning: Add Existing Scenes cancelled: no selected destination",
				string.Join(", ", paths)
			);
			return;
		}
	...
	private void OnCreateScriptFileSelected(string path)
	{
		...
		{
			PushSystemExplorerWarning($"File already exists: {path}", mirrorToDebugLog: false);
			DebugLogger.LogOperation("Warning: Create Script cancelled: file exists", path);
			return;
		}

		if (string.IsNullOrWhiteSpace(GetSelectedSystemName()))
		{
			PushSystemExplorerWarning("Select a system or folder before creating a script.", mirrorToDebugLog: false);
			DebugLogger.LogOperation("Warning: Create Script cancelled: no selected destination", path);
			return;
		}
