...
public partial class SystemExplorerPlugin
{
	...
	private void ToggleItemLock(TreeItem item, bool selectToggledItemAfterBuild = true)
	{
		...
		string selectedMetadataBeforeToggle = selectedItemBeforeToggle?.GetMetadata(0).AsString() ?? "";
		...
		{
			DebugLogger.LogOperation("Toggle Item Lock cancelled: parent system is locked", metadata);
			return;
		}

		if (IsInsideLockedFolder(metadata, includeSelf: false))
		{
			DebugLogger.LogOperation("Toggle Item Lock cancelled: parent folder is locked", metadata);
			return;
		}
	...
	{
		if (
			string.IsNullOrWhiteSpace(exactSystemName)
			|| string.IsNullOrWhiteSpace(exactFolderPath)
		)
			return false;
